...
if has_tool ruff; then
    rm -rf /tmp/t-ruff && mkdir -p /tmp/t-ruff && cd /tmp/t-ruff && no_config
    cp "$FIXTURES/ruff.toml" .
    cp "$FIXTURES/web.py" .
    cp "$FIXTURES/api.py" .
    cp "$FIXTURES/clean.py" .
    cp "$FIXTURES/empty.py" .

    OUT=$(ruff check --output-format=json web.py api.py clean.py empty.py 2>/dev/null | prettyout-ruff || true)
    ...
    OUT=$(ruff check --output-format=json web.py api.py 2>/dev/null | prettyout-ruff || true)
    ...
    check "fixable hint: safe fixes"         "$OUT" "fixable with --fix"
    check "fixable hint: unsafe fixes"       "$OUT" "with --unsafe-fixes"

    with_config ruff max_message_length 10
    OUT=$(ruff check --output-format=json web.py 2>/dev/null | prettyout-ruff || true)
